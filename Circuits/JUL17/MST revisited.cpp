...
int main()
{
	...
	while(q--)
	{
		...
		if(a<b)			c1=mp[make_pair(a,b)],mp.erase(make_pair(a,b));
		else			c1=mp[make_pair(b,a)],mp.erase(make_pair(b,a));;
		...
		if( !((a==tempu && b==tempv) || (a==tempv && b==tempu)))				//removed edge from mst
		{
			...
			g.DFSUtil(a, visited);
			if(visited[tempu]!=visited[tempv] && visited[u]!=visited[v] )	
			{
			...
			sum-=c1;
			if(flag)		g.addEdge(u,v,c);
			else			g.addEdge(tempu,tempv,tempc),tempu=u,tempv=v,tempc=c;
		}
